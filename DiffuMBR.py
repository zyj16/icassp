...
from modules.EVAE import EVAE, evae_loss
...
class MBSSL(nn.Module):
    ...
    def __init__(self, max_item_list, data_config, args):
        ...
        self.all_weights['user_intent'] = Parameter(torch.FloatTensor(self.n_intent, self.emb_dim))
        self.all_weights['item_intent'] = Parameter(torch.FloatTensor(self.n_intent, self.emb_dim))
        self.all_weights['W_u'] = Parameter(torch.FloatTensor(self.emb_dim, self.emb_dim))
        self.all_weights['W_i'] = Parameter(torch.FloatTensor(self.emb_dim, self.emb_dim))
        ...
        self.all_weights['trans_weights_s3'] =  Parameter(torch.FloatTensor(self.emb_dim, self.att_dim))
        self.all_weights['trans_weights_s4'] =  Parameter(torch.FloatTensor(self.att_dim,1))
    ...
    def reset_parameters(self):
        ...
        nn.init.xavier_uniform_(self.all_weights['user_intent'])
        nn.init.xavier_uniform_(self.all_weights['item_intent'])
        ...
        nn.init.xavier_uniform_(self.all_weights['trans_weights_s3'])
        nn.init.xavier_uniform_(self.all_weights['trans_weights_s4'])
    ...
    def forward(self, sub_mats, device):
        ...
        for k in range(0, self.n_layers):
            ...
            embeddings_st = self.dropout(embeddings_st)
            
            all_embeddings = all_embeddings + embeddings_st # ego_embeddings

            embeddings_list_sub1 = []
            for i in range(self.n_relations):
                rela_emb = all_rela_embs[self.behs[i]][k]
                st = time()
                embeddings_ = torch.matmul(self.sub_mat['sub_mat_1%d' % (k + 1)][i],
            ...
            all_embeddings_sub1 = all_embeddings_sub1 + embeddings_st
            ...
            for i in range(self.n_relations):
                ...
                embeddings_ = torch.matmul(self.sub_mat['sub_mat_2%d' % (k + 1)][i],
            ...
            all_embeddings_sub2 = all_embeddings_sub2 + embeddings_st
        ...
        attention = F.softmax(
                    torch.matmul(
                        torch.tanh(torch.matmul(all_embeddings, self.all_weights['trans_weights_s3'])),
                        self.all_weights['trans_weights_s4']
                    ),
                    dim=1
                )
        all_att_embeddings = torch.mul(all_embeddings, attention)
        u_g_embeddings_local, i_g_embeddings_local= torch.split(all_att_embeddings, [self.n_users, self.n_items], 0)
        ...
        for i in range(self.n_relations):
            attention = F.softmax(
                    torch.matmul(
                        torch.tanh(torch.matmul(all_embeddings, self.all_weights['trans_weights_s1'][i])),
                        self.all_weights['trans_weights_s2'][i]
                    ).squeeze(2),
                    dim=1
                ).unsqueeze(1)
            embs_cur_rela = torch.matmul(attention, all_embeddings).squeeze(1)
            embeddings_list.append(embs_cur_rela)
        ego_embeddings= torch.stack(embeddings_list, dim=1)
        u_g_embeddings_global, i_g_embeddings_global = torch.split(ego_embeddings, [self.n_users, self.n_items], 0)
        # all_att_embeddings = self.dropout(all_att_embeddings)

        user_logits = torch.matmul(u_g_embeddings_local.contiguous().view(-1,self.emb_dim), self.all_weights['user_intent'].permute(1,0))
        user_score = torch.softmax(user_logits/math.sqrt(self.emb_dim), dim=1).unsqueeze(-1)

        u_ini_local_embeddings =  (user_score * self.all_weights['user_intent'].unsqueeze(0)).contiguous().view(-1, self.n_relations, self.n_intent, self.emb_dim)
        
        item_logits = torch.matmul(i_g_embeddings_local.contiguous().view(-1,self.emb_dim), self.all_weights['item_intent'].permute(1,0))
        item_score = torch.softmax(item_logits/math.sqrt(self.emb_dim), dim=1).unsqueeze(-1)

        i_ini_local_embeddings =  (item_score * self.all_weights['item_intent'].unsqueeze(0)).contiguous().view(-1, self.n_relations, self.n_intent, self.emb_dim)
        
        # global
        user_logits = torch.matmul(u_g_embeddings_global.contiguous().view(-1,self.emb_dim), self.all_weights['user_intent'].permute(1,0))
        user_score = torch.softmax(user_logits/math.sqrt(self.emb_dim), dim=1).unsqueeze(-1)

        u_ini_global_embeddings =  (user_score * self.all_weights['user_intent'].unsqueeze(0)).contiguous().view(-1, self.n_relations, self.n_intent, self.emb_dim)

        item_logits = torch.matmul(i_g_embeddings_global.contiguous().view(-1,self.emb_dim), self.all_weights['item_intent'].permute(1,0))
        item_score = torch.softmax(item_logits/math.sqrt(self.emb_dim), dim=1).unsqueeze(-1)

        i_ini_global_embeddings =  (item_score * self.all_weights['item_intent'].unsqueeze(0)).contiguous().view(-1, self.n_relations, self.n_intent, self.emb_dim)
        
       
        # sub1 
        all_embeddings_sub1 = self.dropout(all_embeddings_sub1)
        attention = F.softmax(
                    torch.matmul(
                        torch.tanh(torch.matmul(all_embeddings_sub1, self.all_weights['trans_weights_s3'])),
                        self.all_weights['trans_weights_s4']
                    ),
                    dim=1
                )
        all_att_embeddings_sub1 = torch.mul(all_embeddings_sub1, attention)
        # all_att_embeddings_sub1 = self.dropout(all_att_embeddings_sub1)
        u_g_embeddings_local_sub1, i_g_embeddings_local_sub1= torch.split(all_att_embeddings_sub1, [self.n_users, self.n_items], 0)

        user_logits = torch.matmul(u_g_embeddings_local_sub1.contiguous().view(-1,self.emb_dim), self.all_weights['user_intent'].permute(1,0))
        user_score = torch.softmax(user_logits/math.sqrt(self.emb_dim), dim=1).unsqueeze(-1)

        u_ini_local_embeddings_sub1 =  (user_score * self.all_weights['user_intent'].unsqueeze(0)).contiguous().view(-1, self.n_relations, self.n_intent, self.emb_dim)

        item_logits = torch.matmul(i_g_embeddings_local_sub1.contiguous().view(-1,self.emb_dim), self.all_weights['item_intent'].permute(1,0))
        item_score = torch.softmax(item_logits/math.sqrt(self.emb_dim), dim=1).unsqueeze(-1)

        i_ini_local_embeddings_sub1 =  (item_score * self.all_weights['item_intent'].unsqueeze(0)).contiguous().view(-1, self.n_relations, self.n_intent, self.emb_dim)
       
       # sub2
        all_embeddings_sub2 = self.dropout(all_embeddings_sub2)
        attention = F.softmax(
                    torch.matmul(
                        torch.tanh(torch.matmul(all_embeddings_sub2, self.all_weights['trans_weights_s3'])),
                        self.all_weights['trans_weights_s4']
                    ),
                    dim=1
                )
        all_att_embeddings_sub2 = torch.mul(all_embeddings_sub2, attention)
        u_g_embeddings_local_sub2, i_g_embeddings_local_sub2= torch.split(all_att_embeddings_sub2, [self.n_users, self.n_items], 0)

        user_logits = torch.matmul(u_g_embeddings_local_sub2.contiguous().view(-1,self.emb_dim), self.all_weights['user_intent'].permute(1,0))
        user_score = torch.softmax(user_logits/math.sqrt(self.emb_dim), dim=1).unsqueeze(-1)

        u_ini_local_embeddings_sub2 =  (user_score * self.all_weights['user_intent'].unsqueeze(0)).contiguous().view(-1, self.n_relations, self.n_intent, self.emb_dim)

        item_logits = torch.matmul(i_g_embeddings_local_sub2.contiguous().view(-1,self.emb_dim), self.all_weights['item_intent'].permute(1,0))
        item_score = torch.softmax(item_logits/math.sqrt(self.emb_dim), dim=1).unsqueeze(-1)

        i_ini_local_embeddings_sub2 =  (item_score * self.all_weights['item_intent'].unsqueeze(0)).contiguous().view(-1, self.n_relations, self.n_intent, self.emb_dim)
       
       

        u_i_embeddings = torch.sum(u_ini_global_embeddings, dim=2)#+torch.sum((u_ini_local_embeddings),dim=2)
        i_i_embeddings = torch.sum(i_ini_global_embeddings, dim=2)#+torch.sum((i_ini_local_embeddings),dim=2)

        final_u_embeddings = u_i_embeddings  + u_g_embeddings_global#+ u_g_embeddings_local torch.matmul(torch.cat([u_i_embeddings,u_g_embeddings]), self.all_weights['W_u'])
        final_i_embeddings = i_i_embeddings + i_g_embeddings_global # i_g_embeddings_local +  torch.matmul(torch.cat([i_i_embeddings,i_g_embeddings]), self.all_weights['W_i'])
        token_embedding = torch.zeros([1, self.n_relations, self.emb_dim], device=device)
        final_i_embeddings = torch.cat((final_i_embeddings, token_embedding), dim=0)

       
        
       

        return final_u_embeddings, final_i_embeddings, u_ini_global_embeddings, i_ini_global_embeddings, u_ini_local_embeddings_sub1, i_ini_local_embeddings_sub1, u_ini_local_embeddings_sub2, i_ini_local_embeddings_sub2, all_rela_embs
...
class local_ssl(nn.Module):
    ...
    def forward(self, input_u_list, input_i_list, u_sub1_embeddings, i_sub1_embeddings,u_sub2_embeddings,i_sub2_embeddings):
        ...
        ssl_loss = user_cl_loss + item_cl_loss
...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    mlp_model = DNN([64,64], [64,64], args.emb_size, env_size=64, time_type="cat", norm=args.norm,
                    act_func=args.mlp_act_func).to(device)
    evae_model = EVAE(64,64).to(device)
    ...
    for epoch in range(args.epoch):
        ...
        for idx in range(n_batch):
            ...
            model_time = time()
            ua_embeddings, ia_embeddings, ua_embeddings_align, ia_embeddings_align, ua_embeddings_sub1, ia_embeddings_sub1, ua_embeddings_sub2, ia_embeddings_sub2, rela_embeddings = model(sub_mat, device)
            # print('model time: %.1fs' % (time() - model_time))
            # argument ssl loss
            
            ############# diffusion ###############################################
            
            #target_batch_latent = evae_model.encoder(ua_embeddings_align, ia_embeddings_align)# target behavior
            de_loss_list = []
            for i in eval(args.all_beh_idx):
                batch_latent_aux = ua_embeddings_align[:,i,:,:].contiguous().view(-1,64)
                recon, mu, log_std = evae_model(batch_latent_aux)
                kl_div = evae_model.kl_divergence(mu, log_std)
                infer_loss = evae_loss(recon, log_std, kl_div)
                aux_noise_embeddings = evae_model.reparametrize(mu, log_std)
                batch_latent_target = ua_embeddings_align[:,-1:,:].contiguous().view(-1,64)
                terms = diffusion_model.training_losses(mlp_model, batch_latent_target, aux_noise_embeddings, args.reweight)
                diffu_loss = terms["loss"].mean()
                total_loss = diffu_loss+infer_loss
                de_loss_list.append(total_loss)

            de_loss = sum(de_loss_list)
            ...
            for aux_beh in eval(args.aux_beh_idx):
                aux_beh_ssl2_loss = diff_ssl_loss(u_batch_list, i_batch_list, ua_embeddings_align[:,-1,:,:],ia_embeddings_align[:,-1,:,:], ua_embeddings_align[:,aux_beh,:,:], ia_embeddings_align[:,aux_beh,:,:])
            ...
            batch_loss = batch_rec_loss + batch_emb_loss + batch_aru_ssl_loss + batch_ssl2_loss
        ...
        model.eval()
        with torch.no_grad():
            ua_embeddings, ia_embeddings, _, _, _, _, _, _,rela_embeddings = model(sub_mat, device)
        ...
        if args.verbose > 0:
            perf_str = 'Epoch %d [%.1fs + %.1fs]:, recall=[%.5f, %.5f, %.5f, %.5f], ' \
                       'precision=[%.5f, %.5f], hit=[%.5f, %.5f], ndcg=[%.5f, %.5f, %.5f, %.5f]' % \
                       (
                           epoch, t2 - t1, t3 - t2, ret['recall'][0],
                           ret['recall'][1],ret['recall'][2],ret['recall'][3],
