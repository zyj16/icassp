...
class MBSSL(nn.Module):
    ...
    def forward(self, sub_mats, device):
       ...
       # sub2
        ...
        final_u_embeddings = u_i_embeddings + u_g_embeddings_local + u_g_embeddings_global#torch.matmul(torch.cat([u_i_embeddings,u_g_embeddings]), self.all_weights['W_u'])
        final_i_embeddings = i_i_embeddings + i_g_embeddings_local + i_g_embeddings_global #torch.matmul(torch.cat([i_i_embeddings,i_g_embeddings]), self.all_weights['W_i'])
    ...
    *********************************************************
    ...
    for epoch in range(args.epoch):
        ...
        for idx in range(n_batch):
            ...
            batch_loss = batch_rec_loss + batch_emb_loss  + batch_ssl2_loss #+ batch_aru_ssl_loss
        ...
        if args.verbose > 0:
            perf_str = 'Epoch %d [%.1fs + %.1fs]:, recall=[%.5f, %.5f, %.5f, %.5f], ' \
                       ...
                       (
                           ...
                           ret['ndcg'][0], ret['ndcg'][1],ret['ndcg'][2], ret['ndcg'][2])
            print(perf_str)
